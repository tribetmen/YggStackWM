...
int crypto_sign_ed25519_pk_to_curve25519(u8 *curve25519_pk, const u8 *ed25519_pk)
{
  gf x, y, z;
  
  /* Unpack Ed25519 public key (y coordinate) */
  unpack25519(y, ed25519_pk);
  
  /* Formula: u = (1 + y) / (1 - y) */
  A(x, gf1, y);       /* x = 1 + y (numerator) */
  Z(z, gf1, y);       /* z = 1 - y (denominator) */
  inv25519(y, z);     /* y = 1 / (1 - y) */
  M(z, x, y);         /* z = (1 + y) / (1 - y) */
  
  /* Pack result into 32 bytes */
  pack25519(curve25519_pk, z);
  return 0;
}
